...
test_that("Simple interrutpions - No events - (rxode2/mrgsolve)", {
  ...
  results1a <- model %>% simulate(dataset, dest="rxode2", events=events, seed=seed)
  spaghettiPlot(results1a, "CP")

  results1b <- model %>% simulate(dataset, dest="rxode2", events=NULL, seed=seed)
...
test_that("Interruptions at doses times - BW covariate - No events - (rxode2/mrgsolve)", {
  if (skipLongTest) return(TRUE)
  model <- model_library$advan4_trans4
...
test_that("Interruptions at doses times - BW covariate - IOV on KA - No events - (rxode2/mrgsolve)", {
  if (skipLongTest) return(TRUE)
  model <- model_library$advan4_trans4
  equation <- model %>% find(Equation("CL"))
  model <- model %>% replace(Equation("CL", paste0(equation@rhs, "*pow(BW/70, 0.75)")))
  ...
  # table <- dataset %>% export(dest="rxode2", event_related=TRUE)
...
test_that("Simulate initial conditions + events (rxode2/mrgsolve)", {
  if (skipLongTest) return(TRUE)
  ...
  results1 <- model %>% simulate(dataset, dest="rxode2", seed=seed, events=events)
...
test_that("Simulate multiple arms + events (rxode2/mrgsolve)", {
  if (skipLongTest) return(TRUE)
  model <- model_library$advan4_trans4
