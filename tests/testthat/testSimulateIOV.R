...
test_that("Simulate 1000mg QD with IOV on KA (1)", {
  ...
  for (obsTimes in list(obsTimes1, obsTimes2)) {
    ...
    #table_rxode <- dataset %>% export(dest="rxode2", model=model, seed=seed, nocb=TRUE)
    #table_mrgsolve <- dataset %>% export(dest="mrgsolve", model=model, seed=seed, nocb=TRUE)

    results1a <- model %>% simulate(dataset, dest="rxode2", seed=seed, nocbvars="IOV_KA")
    results1b <- model %>% simulate(dataset, dest="rxode2", seed=seed, nocb=TRUE, nocbvars="IOV_KA")
...
test_that("Simulate 1000mg QD with IOV on KA (2) (this test sometimes fails with rxode2 version > 1.0.5 & < 1.1.0)", {
  if (skipLongTest) return(TRUE)
...
test_that("Simulate IOV on F1 (this test always fails with rxode2 version > 1.0.5 & < 1.1.0)", {
  if (skipLongTest) return(TRUE)
...
test_that("Simulate IOV on ALAG1 (this test always fails with rxode2 version > 1.0.5 & < 1.1.0)", {
  if (skipLongTest) return(TRUE)
  ...
  for (startTime in startTimes) {
    ...
    table_rxode <- dataset %>% export(dest="rxode2", model=model, seed=seed, nocb=FALSE)
    table_mrgsolve <- dataset %>% export(dest="mrgsolve", model=model, seed=seed, nocb=FALSE)

    # Bug in rxode2 1.1.0/1.1.1
...
test_that("Simulate IOV on F1", {
  ...
  for (startTime in startTimes) {
    ...
    table_rxode <- dataset %>% export(dest="rxode2", model=model, seed=seed)
    table_mrgsolve <- dataset %>% export(dest="mrgsolve", model=model, seed=seed)

    results1a <- model %>% simulate(dataset, dest="rxode2", seed=1, outvars="F1")
    results1b <- model %>% simulate(dataset, dest="rxode2", seed=1, outvars="F1", nocb=TRUE)
