...
test_that("Add bioavailability to model", {
  ...
  dataset <- Dataset(10) %>% add(Bolus(time=0, amount=1000, compartment=1)) %>%
    add(Bolus(time=24, amount=1000, compartment=1)) %>%
    add(Observations(times=seq(0,48, by=0.5))) %>%
    add(IOV("IOV_F1", distribution=NormalDistribution(0, 0.05)))
...
test_that("Reproduce RxODE bug with a minimalist example", {
  ...
  dataset <- Dataset(10) %>%
    add(Infusion(time=0.01, amount=1000, compartment=1)) %>% # Setting time=0 makes RxODE crash 
