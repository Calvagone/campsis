...
test_that("Reproduce rxode2 bug with a minimalist example", {
  model <- model_library$advan4_trans4 %>% disable("IIV")
  model <- model %>% add(Bioavailability(1, "0.75*exp(IOV_F1)"))
  model <- model %>% add(InfusionDuration(1, "1*exp(ETA_KA)"))

  dataset <- Dataset(10)
  dataset <- dataset %>% add(Infusion(time=0.01, amount=1000, compartment=1)) # Setting time=0 makes rxode2 crash
