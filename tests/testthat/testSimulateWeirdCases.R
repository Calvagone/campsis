...
test_that("Simulate a bolus without observation", {
  ...
  expect_error(model %>% simulate(dataset, dest="rxode2", seed=seed), regexp="Dataset does not contain any observation")
})

test_that("Simulate a bolus with single observation at time 0 (rxode2/mrgsolve)", {
  model <- model_library$advan4_trans4
...
test_that("Simulate a model which is not valid", {
  ...
  expect_error(model %>% simulate(dataset, dest="rxode2"), regexp="name is length 2. Should be 1.")
})
...
test_that("Simulate a dataset which is not valid", {
  ...
  expect_error(model %>% simulate(dataset, dest="rxode2"), regexp="amount is length 2. Should be 1.")
})
