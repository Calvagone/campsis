...
test_that("Weight as a time-varying covariate (NOCB vs LOCF)", {
  ...
  results1 <- model %>% simulate(table, dest="rxode2", seed=seed, nocb=FALSE)
  ...
  results1 <- model %>% simulate(table, dest="rxode2", seed=seed, nocb=TRUE)
  spaghettiPlot(results1, "CP")
...
test_that("NOCB/LOCF should not have any effect on treatment occasion", {
  ...
  # LOCF tests
  results1 <- model %>% simulate(dataset, dest="rxode2", seed=seed, outvars="KA", nocbvars="OCC")
  spaghettiPlot(results1, "CP")
  ...
  # NOCB tests
  results1 <- model %>% simulate(dataset, dest="rxode2", seed=seed, outvars="KA", nocb=TRUE, nocbvars="OCC")
  spaghettiPlot(results1, "CP")
...
test_that("NOCB/LOCF should not have any effect on IOV (e.g. on clearance)", {
  ...
  for(startTime in c(0, 20, 23, 24, 48)) {
    ...
    results1a <- model %>% simulate(dataset, dest="rxode2", seed=seed, nocbvars="IOV_CL")
    spaghettiPlot(results1a, "CP")

    results1b <- model %>% simulate(dataset, dest="rxode2", seed=seed, nocb=TRUE, nocbvars="IOV_CL")
