...
test_that("Body weight as a true time varying covariate (rxode2/mrgsolve)", {
  ...
  results1 <- model %>% simulate(dataset, dest="rxode2", seed=seed, outvars="BW", nocbvars="BW")
...
test_that("Body weight as a true time varying covariate, 2 arms, individual body weights (rxode2/mrgsolve)", {
  model <- model_library$advan2_trans2
  equation <- model %>% find(Equation("CL"))
  model <- model %>% replace(Equation("CL", paste0(equation@rhs, "*pow(BW/70, 0.75)")))
  ...
  results1 <- model %>% disable("IIV") %>% simulate(ds, dest="rxode2", seed=seed, outvars="BW", nocbvars="BW")
