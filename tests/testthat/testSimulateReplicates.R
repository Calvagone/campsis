...
test_that("VPC on CP (predicate)", {
  ...
  results1 <- model %>% simulate(dataset=ds, dest="rxode2", replicates=5, outfun=~PI(.x, output="CP"), seed=seed)
  vpcPlot(results1)
...
test_that("VPC on both CP and Y (function)", {
  ...
  results1 <- model %>% simulate(dataset=ds, dest="rxode2", replicates=5, outfun=fun, seed=seed)
  vpcPlot(results1, scenarios="output") + facet_wrap(~output)

  # vpcOutputRegressionTest(results1, output="Y", filename=regFilename) # Not a good test because seed is controlled by rxode2
  vpcOutputRegressionTest(results1, output="CP", filename=regFilename)
})
...
test_that("Study replication also works with scenarios", {
  ...
  results <- model %>% simulate(dataset=ds, dest="rxode2", replicates=5,
...
test_that("Try/catch works as expected if one replicate fails", {
  ...
  # An warning is thrown by rxode2 for the first replicate
  results_rxode <- expect_warning(simulate(model=model, dataset=dataset, seed=13, replicates=3, outvars="KA", dest="rxode2"),
