...
test_that("NULL outvars (rxode2/mrgsolve)", {
...
test_that("Not NULL outvars (rxode2/mrgsolve)", {
  model <- model_library$advan4_trans4
  ...
  results1 <- model %>% simulate(dataset, dest="rxode2", seed=seed, outvars="KA")
  expect_equal(colnames(results1), c("ID","TIME","ARM","KA","CL","V2","V3","Q","S2","F","CP","OBS_CP","Y","A_DEPOT","A_CENTRAL","A_PERIPHERAL","A_OUTPUT"))
...
test_that("Not NULL outvars + DROP_OTHERS (rxode2/mrgsolve)", {
  model <- model_library$advan4_trans4
  ...
  results1 <- model %>% simulate(dataset, dest="rxode2", seed=seed, outvars=c("KA", "DROP_OTHERS"))
...
test_that("Not NULL outvars from ERROR block + DROP_OTHERS (rxode2/mrgsolve)", {
  model <- model_library$advan4_trans4
  ...
  results1 <- model %>% simulate(dataset, dest="rxode2", seed=seed, outvars=c("Y", "DROP_OTHERS"))
...
test_that("Covariates in outvars can be output well (rxode2/mrgsolve)", {
  model <- model_library$advan4_trans4
  ...
  results1 <- model %>% simulate(dataset, dest="rxode2", seed=seed, outvars=c("Y", "WT", "DROP_OTHERS"))
...
test_that("ETAs in outvars can be output well (rxode2/mrgsolve)", {
  model <- model_library$advan4_trans4
  ...
  results1 <- model %>% simulate(dataset, dest="rxode2", seed=seed, outvars=c("Y", "ETA_KA", "DROP_OTHERS"))
