...
setClass(
  ...
  prototype=prototype(cpu=as.integer(1), slice_parallel=FALSE, slice_size=as.integer(NA),
...
#' @export
Hardware <- function(cpu=1, slice_parallel=FALSE, slice_size=NULL, dataset_parallel=FALSE, dataset_slice_size=500) {
  if (is.null(slice_size)) {
    slice_size <- NA
  }
  return(new("hardware_settings", cpu=as.integer(cpu), slice_parallel=slice_parallel, slice_size=as.integer(slice_size),
