...
VPC <- function(x, scenarios=NULL, level=0.90) {
  ...
  x <- x %>% dplyr::mutate(dplyr::across(c("low", "med", "up"), as.vector))
  
  # Gather data
  if (is.null(scenarios)) {
    x <- x %>% tidyr::gather(key="metric", value="value", -replicate, -time)
  } else {
    x <- x %>% tidyr::gather(key="metric", value="value", -replicate, -time, -dplyr::all_of(scenarios))
  }
