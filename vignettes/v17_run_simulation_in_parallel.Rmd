...
```{r, message=F}
tic()
results <- simulate(model=model, dataset=dataset, dest="mrgsolve", seed=1, outfun=PIOutfun(variable="CONC"), replicates=100)
toc()
```
...
```{r, message=F}
tic()
results <- simulate(model=model, dataset=dataset, dest="mrgsolve", seed=1, outfun=PIOutfun(variable="CONC"), replicates=10, settings=settings)
toc()
```
