...
spaguettiPlot(results, "CP", scenarios="ARM") + ggplot2::facet_wrap(~ARM)
```
...
fun <- function(thetaKA) {
  model <- model %>% replace(Theta(name="KA", index=1, value=thetaKA))
...
fun <- function(combi) {
  model <- model %>% replace(Theta(name="KA", index=1, value=combi$THETA_KA))
