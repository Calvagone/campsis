...
```{r several_protocols_with_arms, fig.align='center', fig.height=4, fig.width=8}
...
results <- model %>% simulate(dataset=Dataset() %>% add(c(arm1, arm2, arm3)), dest="RxODE", seed=1)
spaghettiPlot(results, "CP", "ARM") + ggplot2::facet_wrap(~ARM)
```

Using scenarios:

```{r several_protocols_with_scenarios, fig.align='center', fig.height=4, fig.width=8}
model <- model_library$advan4_trans4
...
results <- model %>% simulate(dataset=dataset, dest="RxODE", scenarios=scenarios, seed=1)
...
```{r scenarios_make_theta_ka_vary, fig.align='center', fig.height=4, fig.width=8}
model <- model_library$advan4_trans4
...
results <- model %>% simulate(dataset=ds, dest="RxODE", scenarios=scenarios, seed=1)
...
```{r scenarios_compare_bw_distributions , fig.align='center', fig.height=6, fig.width=8}
