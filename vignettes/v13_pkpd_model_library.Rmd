...
```{r direct_effect_model, fig.align='center', fig.height=4, fig.width=8}
library(campsis)
dataset <- Dataset(25) %>% 
...
```{r effect_compartment_model, fig.align='center', fig.height=4, fig.width=8}
library(campsis)
dataset <- Dataset(25) %>% 
...
```{r transit_compartment_model, fig.align='center', fig.height=4, fig.width=8}
library(campsis)
dataset <- Dataset(25) %>% 
...
Load the indirect-response model (IRM) from the model library:

```{r}
pd <- model_library$irm_kin_stimulation
pd
```
...
```{r indirect_response_model, fig.align='center', fig.height=4, fig.width=8}
library(campsis)
dataset <- Dataset(25) %>% 
  add(Bolus(time=0, amount=1000, compartment=1, ii=12, addl=35)) %>%
  add(Observations(times=0:1000))
