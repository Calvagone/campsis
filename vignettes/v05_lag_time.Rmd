...
results_alag <- model %>% simulate(dataset=ds1, dest="rxode2", seed=1)
results_no_alag <- model_library$advan4_trans4 %>% simulate(dataset=ds1, dest="rxode2", seed=1)
...
```{r}
ds2 %>% export(dest="rxode2") %>% dosingOnly() %>% head()
```
...
results_alag <- model_library$advan4_trans4 %>% simulate(dataset=ds2, dest="rxode2", seed=1)
