...
```{r event_bw_as_time_varying_covariate_scenarios, fig.align='center', fig.height=4, fig.width=8}
scenarios <- Scenarios() %>%
...
Finally, time-varying covariates can also be individualised by specifying an ID column:

```{r event_bw_as_time_varying_covariate_by_id, fig.align='center', fig.height=4, fig.width=8}
dataset <- Dataset(2) %>%
  add(Bolus(time=0, amount=1000)) %>%
  add(Observations(times=seq(0,30,by=0.1))) %>%
